...
export const ScrollContainer = forwardRef<HTMLDivElement, ScrollContainerProps>(
  function ScrollContainer({ children, className, wrapperClassName }, ref) {
    ...
    const updateThumb = useCallback(() => {
      ...
      if (!hasScroll) {
        setThumb({ top: 0, height: 0, hasScroll: false });
        return;
      }
      ...
      setThumb({ top: thumbTop, height: thumbHeight, hasScroll: true });
    }, []);
    ...
    }, [updateThumb, showThumb, scheduleHide, dragging]);
        ...
        {thumb.hasScroll && (
          ...
          >
            <div
              ...
              aria-valuenow={scrollRef.current ? Math.round(
                (scrollRef.current.scrollTop / Math.max(1, scrollRef.current.scrollHeight - scrollRef.current.clientHeight)) * 100,
              ) : 0}
              aria-valuemin={0}
              aria-valuemax={100}
