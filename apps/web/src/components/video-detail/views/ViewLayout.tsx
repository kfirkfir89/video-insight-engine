...
// ─────────────────────────────────────────────────────
// sidebarMainOrFallback — sidebar+main or decomposed fallback
// ─────────────────────────────────────────────────────

/**
 * When both sidebar and main content exist → pair them in a sidebar-main row.
 * Otherwise → return the fallback sections (typically the decomposed individual items).
 *
 * This eliminates the repeated if(hasSidebar && hasMain)/else pattern across views.
 */
export function sidebarMainOrFallback(
  ...
  if (sidebar != null && main != null) {
    return [buildPairedSection(
      { width: 'sidebar', node: sidebar },
      { width: 'main', node: main },
      key,
    )];
  }
  return fallback;
