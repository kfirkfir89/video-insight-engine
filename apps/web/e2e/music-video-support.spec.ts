...
test.describe("Music Video - Responsive Transitions", () => {
  test("layout transitions correctly between desktop and tablet", async ({
    ...
    await expect(stickyNav).toBeVisible({ timeout: 5000 });
  });
});
...
test.describe("Music Video - Accessibility", () => {
  ...
  test("keyboard navigation activates music video chapter", async ({
    ...
    await expect(secondChapter).toHaveAttribute("data-active", "true", { timeout: 5000 });
  });
});
...
test.describe("Music Video - Interactions", () => {
  ...
  test("clicking chapter nav activates target chapter", async ({
    ...
    // Verify the chapter nav item becomes active
    await expect(grandFinale).toHaveAttribute("data-active", "true", { timeout: 5000 });
...
test.describe("Music Video - Error State", () => {
  test("displays error when API returns 500", async ({
    authenticatedPage: page,
  }) => {
    // Override video detail route to return 500
    await page.route(/\/api\/videos\/video-music-1$/, (route) => {
      route.fulfill({
        status: 500,
        contentType: "application/json",
