...
const mockRecipeOutput = {
  ...
  enrichment: null,
  // v2: assembled tabs with component-addressed props
  assembledTabs: [
    {
      id: "ingredients",
      label: "Ingredients",
      ...
      props: {
        items: [
          { label: "500g Pasta", group: "Main", emoji: "\u{1F35D}" },
          { label: "2 tbsp Olive Oil", group: "Sauce", emoji: "\u{1FAD2}" },
          { label: "3 cloves Garlic", group: "Sauce", emoji: "\u{1F9C4}" },
          { label: "1 bunch Fresh Basil", note: "For garnish", emoji: "\u{1F33F}" },
        ],
    ...
    },
    {
      ...
      props: {
        steps: [
          { title: "Cook pasta", body: "Boil water and cook pasta according to package directions.", duration: "10 min", tip: "Salt the water generously" },
          { title: "Heat oil", body: "Heat olive oil in a large pan over medium heat.", duration: "2 min" },
          { title: "Add garlic", body: "Add garlic and cook until fragrant, about 1 minute.", duration: "1 min", tip: "Don't let it burn" },
        ],
        mode: "scrollable",
      },
    },
...
test.describe("Output Layout", () => {
  test.describe("Desktop layout hierarchy", () => {
    ...
    test("should render key takeaways section", async ({ authenticatedPage: page }) => {
      ...
      // Key takeaways should render at bottom (heading is uppercase)
  ...
  test.describe("Recipe output type", () => {
    test("should render recipe with correct structure", async ({ authenticatedPage: page }) => {
      ...
      // Ingredients rendered as checklist (v2 assembled tab)
      await expect(page.getByText(/500g Pasta/)).toBeVisible();
    });
    ...
    test("recipe tabs should switch correctly", async ({ authenticatedPage: page }) => {
      ...
      // Steps content should be visible (v2 assembled tabs use step_player)
