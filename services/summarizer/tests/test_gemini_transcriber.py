...
class TestDownloadAudioRawSync:
    ...
    @patch("src.services.gemini_transcriber.yt_dlp.YoutubeDL")
    ...
    @patch("src.services.gemini_transcriber.time.sleep")
    @patch("src.services.gemini_transcriber.yt_dlp.YoutubeDL")
...
class TestGeminiEmptyResponseHandling:
    ...
    async def test_no_candidates_raises_transcript_error(
        ...
        mock_response = MagicMock()
        mock_response.text = property(lambda _: (_ for _ in ()).throw(ValueError("no candidates")))
        type(mock_response).text = property(lambda _: (_ for _ in ()).throw(ValueError("no candidates")))
        mock_client.aio.models.generate_content = AsyncMock(return_value=mock_response)
        mock_client_class.return_value = mock_client
