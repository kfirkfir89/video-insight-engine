...
class TestDownloadAudioSync:
    ...
    @patch("src.services.whisper_transcriber.time.sleep")
    @patch("src.services.whisper_transcriber.yt_dlp.YoutubeDL")
...
class TestTranscribeChunkedSync:
    ...
    def test_merges_text_from_all_chunks(self, mock_transcribe, tmp_path):
    ...
    @patch("src.services.whisper_transcriber._transcribe_sync")
    def test_adjusts_segment_timestamps_by_offset(self, mock_transcribe, tmp_path):
    ...
    @patch("src.services.whisper_transcriber._transcribe_sync")
    def test_handles_chunks_without_segments(self, mock_transcribe, tmp_path):
    ...
    @patch("src.services.whisper_transcriber._transcribe_sync")
    def test_single_chunk_works(self, mock_transcribe, tmp_path):
    ...
    @patch("src.services.whisper_transcriber._transcribe_sync")
    def test_passes_is_music_to_each_chunk(self, mock_transcribe, tmp_path):
...
class TestClassifyDownloadError:
    ...
    def test_partial_read_classified_as_download_error(self):
        ...
        assert _classify_download_error(msg) == ErrorCode.DOWNLOAD_ERROR

    def test_timeout_classified_as_download_error(self):
        """Test that timeout errors map to DOWNLOAD_ERROR."""
        assert _classify_download_error("Connection timed out") == ErrorCode.DOWNLOAD_ERROR

    def test_unknown_error_classified_as_download_error(self):
        """Test that unknown errors default to DOWNLOAD_ERROR."""
        assert _classify_download_error("Something went wrong") == ErrorCode.DOWNLOAD_ERROR
...
class TestDownloadRetryBehavior:
    ...
    def test_exhausts_retries_then_raises(self, mock_ydl_class, mock_sleep):
        ...
        assert exc_info.value.code == ErrorCode.DOWNLOAD_ERROR
        assert mock_ydl.download.call_count == _MAX_DOWNLOAD_ATTEMPTS
