...
from src.services.whisper_transcriber import _classify_download_error
...
def _get_mime_type(audio_path: Path) -> str:
    ...
    if mime is None:
        logger.warning(f"Unknown audio extension '{ext}', defaulting to audio/webm")
        return "audio/webm"
    return mime
...
def _download_audio_raw_sync(video_id: str) -> Path:
    ...
    logger.info(f"Downloaded raw audio: {audio_path.name} ({file_size_mb:.1f}MB)")
    return audio_path
...
def _recover_truncated_json(text: str, start_idx: int) -> list[dict]:
    ...
    # Find the last complete object by looking for the last "},"  or "}" that
    # closes a segment. We search for '}' from the end and try progressively
    # shorter slices until we get valid JSON.
...
async def transcribe_with_gemini(
    ...
    try:
        ...
        logger.info(
            f"Starting Gemini transcription for {video_id} "
            f"(music_mode={'ON' if is_music else 'OFF'})"
        )
        ...
        logger.info(f"File uploaded: {upload_result.name} (uri: {upload_result.uri})")
        ...
        try:
            ...
            logger.debug(f"Deleted uploaded file: {upload_result.name}")
        except Exception as e:
            logger.debug(f"Failed to delete uploaded file (non-critical): {e}")
        ...
        for seg in raw_segments:
            try:
                ...
                logger.warning(f"Skipping malformed Gemini segment: {e} | {seg}")


        raw_text = " ".join(seg.text for seg in segments)

        logger.info(
            f"Gemini transcription complete: "
            f"{len(raw_text)} chars, {len(segments)} segments"
        )
